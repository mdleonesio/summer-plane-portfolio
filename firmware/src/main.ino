...
File logFile;
const int chipSelect = BUILTIN_SDCARD;  // For Teensy 4.1
...
const int CH_YAW      = 0;
const int CH_SWA      = 4;  // Mission start
const int CH_KNOB1    = 8;  // Use for ramp time
const int CH_KNOB2    = 9;  // Use for climb duration or target pitch
...
float targetPitchClimb = 13; // Can be made dynamic with knob
float targetYaw = 0;
...
unsigned long throttleRampDuration = 3000;  // CH8 maps this
unsigned long elevatorRampDuration = 3000;  // CH9 maps this
unsigned long climbStartTime = 0;   // time when CLIMB_CONTROL begins
unsigned long climbDuration = 10000;         // Optional — could also be knob-controlled
...
void loop() {
  updateMissionState();  // Check if SWA was flipped and update currentPhase
  runMission();          // Runs correct logic for each mission phase
  writeOutputs();  
  checkLoggingSwitch();     // Send final servo values
  logFlightData();       // Optional: phase-aware SD logging
...
void updateMissionState() {
  ...
  if (switchArmed && !lastSwitchState) {
    ...
    // Lock in throttle ramp time from knob
...
void runThrottleRamp(unsigned long t) {
  ...
  // Enable rudder control once above 1500
  if (throttleOut > 1500) {
    if (!takeoffYawInit) {
...
void runElevatorRamp(unsigned long t) {
  imu::Vector<3> euler = bno.getVector(Adafruit_BNO055::VECTOR_EULER);
  float currentPitch = -euler.z();  // Negative because BNO Z is backward
  ...
  // Get ramp duration from knob
  int rawKnob = iBus.readChannel(CH_KNOB2);
  unsigned long elevatorRampDuration = map(rawKnob, 1000, 2000, 1000, 10000);  // 1–10s

  // Wait until throttle is high before starting elevator ramp
  ...
  if (!elevatorFrozen && elevatorRampStarted) {
    ...
    // Freeze once pitch exceeds threshold
    if (currentPitch > 8.0) {
  ...
  if (elevatorFrozen) {
    ...
    if (millis() - freezeStartTime > 1500) {
      ...
      climbStartTime = millis();  // ✅ now it's in the correct place
...
void runClimbControl() {
  ...
  pitch = -euler.z();   // Negative because of BNO orientation
  pitchRate = gyro.x();
  yaw = euler.x();
  yawRate = gyro.z();
  ...
  pitchOut = constrain(90 - elevatorCmd, 60, 120);  // Adjust range as needed
  ...
  // Throttle remains high during climb
  throttleOut = 1800;
...
void runCruiseControl() {
  ...
  // === PITCH CONTROL: hold level ===
  float targetPitchCruise = 0;  // Or adjust if you want a slight nose-up
  float pitchError = targetPitchCruise - pitch;
  float elevatorCmd = kP * pitchError - kD * pitchRate;
  pitchOut = constrain(90 - elevatorCmd, 60, 120);

  // === YAW CONTROL: hold heading ===
  ...
  // === Throttle: reduce to cruise power ===
  throttleOut = 1500;  // Tune this — depends on your airframe
...
void resetMissionState() {
  // Reset all mission control flags and timers
  elevatorRampStarted = false;
  elevatorFrozen = false;
...
void logFlightData() {
  ...
  if (!isLogging || now - lastLogTime < 50) return;  // 20 Hz logging
  ...
  logFile.flush();  // Optional but safer during testing
}
